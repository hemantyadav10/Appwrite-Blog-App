...
function SideNavbar() {
  ...
  return (
    !isAuthenticated ? <Navigate to={'/login'} /> :
      <Container>
        <div className='flex flex-col w-full bg-white sm:flex-row dark:bg-[#0d1117] transition-colors max-w-screen-2xl mx-auto'>
          ...
          <section className={`  min-w-[250px] py-6 dark:border-r-[rgb(48,54,61)] sm:flex flex-col gap-12 lg:pl-24 min-h-[calc(100vh-72px)] sm:pl-6 max-w-[300px] lg:w-full color fixed bg-white z-40 sm:z-0  ${!showMenu && ' hidden '}  dark:bg-[#0d1117] transition-colors  shadow-lg sm:shadow-none bottom-0 top-16 left-0 sm:relative sm:top-0`} >
                    ...
                    {item.subHeadings.map((subItems, j) => (
                      <li key={j}>
                        <NavLink
                          ...
                          to={subItems.slug}
                          className={({ isActive }) => `${isActive ? 'bg-gray-100 border-l-[3px] border-l-[#1f1f1f] dark:bg-[#262c36] dark:border-l-indigo-400 color ' : ' hover:bg-gray-100  active:bg-gray-200 dark:hover:bg-[#262c36] dark:active:bg-[#2a313c] medium_color hover:text-[#1f1f1f] dark:hover:text-[#E6EDF3]'} flex items-center gap-3  py-3 px-4 transition-colors`}
                        >
